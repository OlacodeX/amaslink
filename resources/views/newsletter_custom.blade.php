...
<div class="container create">
   ...
   {!! Form::open(['action' => 'SuperadminController@send_external', 'method' => 'POST']) !!}
    <div class="form-group">
        ...
        {{Form::text('title', '', [ 'class' => 'form-control', 'placeholder' => 'Mail Subject'])}}
    </div>
    <div class="form-group">
        <!--This is the lable for the field. The first parameter is the lable for, while the second is the name it will carry--->
        {{Form::label('recipients', 'Recipients')}}
        <!--This is the input field with type=text, name=title, value='' since it is a text field, then bootstrap class and then placeholder--->
        {{Form::textarea('recipient', '', [ 'class' => 'form-control', 'placeholder' => 'Enter email addresses of recipients separated by comas(,)'])}}
    </div>
    <div class="form-group">
        <!--This is the lable for the field. The first parameter is the lable for, while the second is the name it will carry--->
        {{Form::label('content', 'Content')}}
        <!--This is the input field with type=text, name=title, value='' since it is a text field, then bootstrap class and then placeholder--->
        {{Form::textarea('body', '', [ 'class' => 'form-control', 'placeholder' => 'Mail Content'])}}
    </div>
    {{Form::submit('Send Newsletter', ['class' => 'btn btn-success btn-md pull-left', 'style' => 'text-transform:uppercase;'])}}
    {!! Form::close() !!}
