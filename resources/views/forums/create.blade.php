...
@section('page_title')
{{config('app.name')}}- Best Free Classified Ads | Start Discussion
@endsection
<style>
    #main {
...
<div class="container create">
    <h3 class="create title">Start <span>Discussion</span></h3>
    @include('inc.messages')
    <!---If file upload is involved always add enctype to your opening
        form tag and set it to multipart/form-data--->
   {!! Form::open(['action' => 'ForumController@store', 'method' => 'POST']) /** The action should be the block of code in the store function in PostsController
   **/ !!}
    ...
    <div class="form-group">
        ...
        <!--This is the input field with type=text, name=title, value='' since it is a text field, then bootstrap class and then placeholder--->
        {{Form::text('topic', '', [ 'class' => 'form-control', 'placeholder' => 'Discussion Topic'])}}
    </div>
    {{Form::submit('Start Discussion', ['class' => 'btn btn-success btn-md pull-left', 'style' => 'text-transform:uppercase;'])}}
   {!! Form::close() !!}
